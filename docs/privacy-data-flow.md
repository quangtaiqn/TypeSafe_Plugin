...
- OAuth bearer tokens are validated against the configured issuer, audience, JWKS, expiry, scope, and subject allowlist. JWKS fetches are bounded by timeout and response size, and pre-auth attempts are rate-limited. The upstream TypeSafe key is never returned to the client.
